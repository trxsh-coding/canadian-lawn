...
const PACKAGE_WEIGHTS = [1, 5, 10];
const PACKAGE_DISCOUNTS = { 1: 1, 5: 0.93, 10: 0.85 };

const buildLawnPackages = (basePrice) =>
  PACKAGE_WEIGHTS.map((weight) => ({
    weight,
    price: Math.round(basePrice * weight * PACKAGE_DISCOUNTS[weight]),
  }));

const LANDING_PRESETS = [
  { april: true, may: true, august: true, september: true },
  { april: true, may: true, june: true, august: true, september: true },
  { may: true, june: true, august: true, september: true },
];

const buildLanding = () => faker.helpers.arrayElement(LANDING_PRESETS);

async function seedProducts() {
  await run('продукты', products, ({ name, type, price }) => {
    const basePrice = faker.number.int(price);
    const isLawn = type === 'lawn' || type === 'lawn-mix';
    ...
    return post('/products', {
      ...
      ...(isLawn
        ? { lawn: { package: buildLawnPackages(basePrice), landing: buildLanding() } }
        : {}),
    });
  });
}
...
async function seedFaq() {
  const faqs = [
    // Посев и подготовка
    { title: 'Когда лучше всего сеять газон?',            slug: 'kogda-seyat-gazon',          blocks: '<p>Оптимальное время — <strong>весна (апрель–май)</strong> или <strong>ранняя осень (август–сентябрь)</strong>. Весной почва прогрета и достаточно влаги, осенью меньше сорняков и мягче солнце.</p>' },
    { title: 'Как подготовить почву перед посевом?',       slug: 'podgotovka-pochvy',          blocks: '<p>Перекопайте участок на 15–20 см, уберите корни сорняков и камни, выровняйте граблями. При тяжёлой глинистой почве добавьте песок, при бедной — чернозём или компост.</p><ul><li>Удаление старой дернины и мусора</li><li>Планировка и трамбовка поверхности</li><li>Внесение стартового удобрения</li></ul>' },
    { title: 'Какая норма высева семян на м²?',            slug: 'norma-vyseva-semyan',        blocks: '<p>В среднем — <strong>20–40 г/м²</strong> в зависимости от травосмеси. Точная норма указана на упаковке конкретного продукта — плотность посева влияет на густоту будущего газона.</p>' },
    { title: 'Нужно ли перекапывать участок под газон?',   slug: 'perekopka-uchastka',         blocks: '<p>Да, если почва плотная или заросшая. Перекопка на штык лопаты насыщает грунт кислородом и облегчает прорастание корней. На рыхлой окультуренной земле достаточно рыхления граблями.</p>' },
    { title: 'Сколько всходят семена газонной травы?',    slug: 'skolko-vskhodyat-semena',    blocks: '<p>Первые всходы появляются через <strong>7–14 дней</strong> при регулярном поливе и температуре почвы от +10°C. Полная густота формируется за 4–6 недель.</p>' },
    { title: 'Можно ли сеять газон вручную без сеялки?',  slug: 'posev-vruchnuyu',            blocks: '<p>Можно. Разделите семена на две части и сейте крест-накрест — вдоль и поперёк участка, это даёт более равномерное распределение. После посева слегка заделайте семена граблями и прикатайте.</p>' },
    { title: 'Что делать если газон взошёл неравномерно?', slug: 'neravnomernye-vskhody',      blocks: '<p>Проведите подсев на проплешинах: разрыхлите почву, добавьте семена той же смеси, присыпьте тонким слоем земли и полейте. Обычно причина — пересыхание верхнего слоя в первые недели после посева.</p>' },

    // Полив
    { title: 'Сколько воды нужно газону?',                 slug: 'poliv-gazona',               blocks: '<p>В жаркий период — <strong>2–3 раза в неделю</strong> по 20–30 л на м². Лучшее время для полива — раннее утро или вечер, когда испарение минимально.</p>' },
    { title: 'Как часто поливать молодой газон?',          slug: 'poliv-molodogo-gazona',      blocks: '<p>В первые 2–3 недели после посева поливайте <strong>ежедневно</strong> мелким дождеванием, не допуская пересыхания верхнего слоя почвы. После укоренения переходите на обычный режим полива.</p>' },
    { title: 'Можно ли поливать газон в жару днём?',       slug: 'poliv-v-zharu-dnyom',        blocks: '<p>Не рекомендуется — капли воды работают как линзы и могут вызвать ожоги листьев, а вода быстро испаряется. Поливайте <strong>до 10 утра или после 18 часов</strong>.</p>' },
    { title: 'Нужна ли автоматическая система полива?',    slug: 'avtomaticheskiy-poliv',      blocks: '<p>Не обязательна, но заметно упрощает уход, особенно на больших площадях. Система с таймером обеспечивает регулярность полива и экономит воду за счёт точной дозировки.</p>' },

    // Стрижка
    { title: 'Когда делать первую стрижку?',               slug: 'pervaya-strizhka',           blocks: '<p>При достижении травой <strong>8–10 см</strong>. Срезайте не более 1/3 высоты за один раз. Ножи газонокосилки должны быть острыми, чтобы не повредить молодые корни.</p>' },
    { title: 'На какую высоту стричь газон летом?',        slug: 'vysota-strizhki-letom',      blocks: '<p>Оптимальная высота — <strong>4–6 см</strong>. В засушливую жару можно оставлять траву чуть выше (6–7 см) — это уменьшает испарение влаги из почвы.</p>' },
    { title: 'Как часто нужно стричь газон?',              slug: 'chastota-strizhki',          blocks: '<p>В сезон активного роста (май–июль) — <strong>раз в 5–7 дней</strong>. Весной и осенью, когда рост замедляется, достаточно одного раза в 10–14 дней.</p>' },
    { title: 'Что делать со скошенной травой — убирать или оставлять?', slug: 'skoshennaya-trava', blocks: '<p>Короткую сухую скошенную траву можно оставлять — она перегнивает и служит естественным удобрением. Влажную и длинную траву лучше убирать граблями, чтобы избежать плесени и появления жёлтых пятен.</p>' },
    { title: 'Как правильно выбрать газонокосилку?',       slug: 'vybor-gazonokosilki',        blocks: '<p>Для участков до 6 соток подойдёт электрическая или аккумуляторная косилка, для больших территорий — бензиновая или райдер. Учитывайте рельеф: для неровных участков нужны колёса большего диаметра.</p>' },

    // Удобрения и уход
    { title: 'Нужно ли удобрять газон?',                   slug: 'udobrenie-gazona',           blocks: '<p>Да. Весной вносите <strong>азотные удобрения</strong> для роста зелёной массы, летом — комплексные NPK, осенью — фосфорно-калийные для укрепления корней перед зимой.</p>' },
    { title: 'Какие удобрения использовать весной и осенью?', slug: 'udobreniya-vesna-osen',   blocks: '<p>Весной — с высоким содержанием азота (N), стимулирующего рост. Осенью — с преобладанием фосфора и калия (P, K), повышающих морозостойкость и укрепляющих корневую систему.</p>' },
    { title: 'Что такое аэрация и зачем она нужна?',       slug: 'aeratsiya-gazona',           blocks: '<p>Прокалывание дёрна на 8–10 см специальными вилами или аэратором. Улучшает доступ воздуха, воды и удобрений к корням, устраняет застой влаги. Проводится <strong>раз в год</strong>, обычно осенью.</p>' },
    { title: 'Что такое скарификация газона?',             slug: 'skarifikatsiya-gazona',      blocks: '<p>Удаление войлока — слоя отмерших стеблей и мха у основания травы. Проводится весной или осенью специальным скарификатором и улучшает воздухообмен в дернине.</p>' },
    { title: 'Нужно ли известковать почву под газон?',     slug: 'izvestkovanie-pochvy',       blocks: '<p>Известкование нужно, если почва кислая (pH ниже 6). Проверить кислотность можно тест-полоской. Известь вносят осенью, отдельно от азотных удобрений.</p>' },

    // Проблемы
    { title: 'Почему газон желтеет?',                      slug: 'gazon-zhelteyet',            blocks: '<p>Основные причины: <strong>нехватка азота</strong>, пересыхание почвы, грибковые заболевания или повреждение реагентами и солями. Определите причину по характеру пятен и устраните её.</p>' },
    { title: 'Появились проплешины на газоне — что делать?', slug: 'proplesiny-na-gazone',     blocks: '<p>Разрыхлите землю на проплешине, подсыпьте плодородный грунт, посейте семена той же травосмеси и полейте. Первые 2 недели поливайте ежедневно, как молодой газон.</p>' },
    { title: 'Как бороться с сорняками?',                  slug: 'borba-s-sornyakami',         blocks: '<p>Регулярная стрижка подавляет большинство однолетних сорняков. Для устойчивых многолетников используйте точечную обработку селективными гербицидами, которые не вредят злаковым травам.</p>' },
    { title: 'Появился мох на газоне — почему?',           slug: 'moh-na-gazone',              blocks: '<p>Мох появляется из-за избыточной влажности, затенения, кислой почвы или слишком низкой стрижки. Проведите скарификацию, улучшите дренаж и при необходимости раскислите почву известью.</p>' },
    { title: 'Газон поражён грибком — что делать?',        slug: 'gribok-na-gazone',           blocks: '<p>Уберите поражённые участки, сократите полив и улучшите проветривание (проведите аэрацию). В сложных случаях используйте фунгициды, предназначенные для газонных трав.</p>' },
    { title: 'На газоне завелись кроты — как избавиться?',  slug: 'kroty-na-gazone',            blocks: '<p>Эффективны ультразвуковые отпугиватели и вибрационные колышки. Народный способ — заглубить по периметру шумящие предметы или использовать резко пахнущие вещества у нор.</p>' },

    // Сезонность
    { title: 'Как ухаживать за газоном зимой?',            slug: 'gazon-zimoy',                blocks: '<p>Избегайте хождения по замёрзшей или заснеженной траве — это ломает стебли. В ноябре проведите последнюю подкормку фосфорно-калийным удобрением, молодые посевы укройте лутрасилом.</p>' },
    { title: 'Что делать с газоном весной после зимы?',    slug: 'gazon-vesnoy',               blocks: '<p>Уберите прошлогоднюю листву и мусор веерными граблями, проведите первую стрижку и внесите азотное удобрение. При необходимости подсейте проплешины, оставшиеся после зимы.</p>' },
    { title: 'Как подготовить газон к зиме осенью?',       slug: 'podgotovka-k-zime',          blocks: '<p>В сентябре–октябре внесите фосфорно-калийное удобрение, проведите аэрацию, уберите опавшую листву и снизьте высоту последней стрижки до 4–5 см, чтобы трава не выпревала под снегом.</p>' },

    // Выбор продукции
    { title: 'Чем отличается газонная трава от газонной смеси?', slug: 'trava-i-smes-otlichie', blocks: '<p><strong>Газонная трава</strong> — семена одного вида (например, овсяницы). <strong>Газонная смесь</strong> — сочетание нескольких видов трав, что делает покрытие более устойчивым к разным условиям — тени, засухе, вытаптыванию.</p>' },
    { title: 'Можно ли сеять газон в тени?',               slug: 'gazon-v-teni',               blocks: '<p>Да, но нужны <strong>теневыносливые смеси</strong> на основе овсяницы красной и мятлика дубравного. В густой тени густота травостоя всегда будет ниже, чем на солнце.</p>' },
    { title: 'Какой газон выбрать для собак и активного отдыха?', slug: 'gazon-dlya-sobak',    blocks: '<p>Подойдут <strong>спортивные смеси</strong> с райграсом пастбищным — он быстро восстанавливается после вытаптывания и повреждений. Такие газоны выдерживают повышенную нагрузку.</p>' },
    { title: 'Какой газон выбрать для дачи с минимальным уходом?', slug: 'gazon-dlya-dachi',   blocks: '<p>Обратите внимание на <strong>неприхотливые смеси эконом-класса</strong> и мавританские (луговые) газоны — они реже нуждаются в стрижке и хорошо переносят нерегулярный уход.</p>' },
    { title: 'Рулонный газон или посев семенами — что лучше?', slug: 'rulonnyy-ili-semena',    blocks: '<p>Рулонный газон даёт результат сразу, но стоит дороже. Посев семенами дешевле и позволяет точно подобрать травосмесь под условия участка, хотя полноценный газон формируется за 1–2 сезона.</p>' },

    // Магазин и заказ
    { title: 'Какой срок годности у семян газонной травы?', slug: 'srok-godnosti-semyan',      blocks: '<p>При правильном хранении в сухом прохладном месте семена сохраняют всхожесть <strong>2–3 года</strong>. Срок годности партии указан на упаковке.</p>' },
    { title: 'Даёте ли вы гарантию всхожести семян?',      slug: 'garantiya-vskhozhesti',       blocks: '<p>Да, вся продукция проходит лабораторный контроль всхожести перед продажей. При соблюдении агротехники (сроки, полив, подготовка почвы) всхожесть составляет не менее 90%.</p>' },
    { title: 'Как оформить доставку по России?',           slug: 'dostavka-po-rossii',          blocks: '<p>Доставка осуществляется через сеть партнёров и транспортные компании в любую точку страны в течение <strong>3–5 рабочих дней</strong>. Оформить заказ можно через корзину на сайте.</p>' },
    { title: 'Можно ли вернуть семена, если не подошли?',  slug: 'vozvrat-semyan',              blocks: '<p>Возврат возможен, если упаковка не вскрыта и товарный вид сохранён, в течение 14 дней с момента покупки согласно закону о защите прав потребителей.</p>' },
    { title: 'Есть ли скидки при оптовом заказе для партнёров?', slug: 'skidki-optom',           blocks: '<p>Да, для партнёрской сети действует гибкая система скидок в зависимости от объёма заказа. Подробности уточняйте у менеджера через раздел «Партнёрам» на сайте.</p>' },
  ];
...
async function seedPartners() {
  const cities = [
    ...
    { city: 'Уфа',             lat: 54.7388, lng: 55.9721 },
    { city: 'Пермь',           lat: 58.0105, lng: 56.2502 },
    { city: 'Воронеж',         lat: 51.6755, lng: 39.2088 },
  ];
  ...
  const partners = [
    ...
    { name: 'МосГазон',              city: cities[0],  phone: '+7 (495) 321-00-12', working_hours: 'Пн–Сб 9:00–19:00',  open_on_weekends: true,  rank: 3, isFeature: true  },
    { name: 'СПб Газон',             city: cities[1],  phone: '+7 (812) 111-22-33', working_hours: 'Пн–Сб 9:00–19:00',  open_on_weekends: true,  rank: 1, isFeature: true  },
    ...
    { name: 'Екат-Газон',            city: cities[2],  phone: '+7 (343) 888-77-66', working_hours: 'Пн–Сб 9:00–20:00',  open_on_weekends: true,  rank: 2, isFeature: true  },
    ...
    { name: 'НовосибГрин',           city: cities[3],  phone: '+7 (383) 100-20-30', working_hours: 'Пн–Пт 9:00–18:00',  open_on_weekends: false, rank: 2, isFeature: false },
    { name: 'КазаньГазон Про',       city: cities[4],  phone: '+7 (843) 333-44-55', working_hours: 'Пн–Сб 9:00–17:00',  open_on_weekends: true,  rank: 1, isFeature: true  },
    ...
    { name: 'НижегородГрин',         city: cities[6],  phone: '+7 (831) 200-30-40', working_hours: 'Пн–Сб 9:00–19:00',  open_on_weekends: true,  rank: 2, isFeature: false },
    { name: 'СамараГрин',            city: cities[7],  phone: '+7 (846) 999-00-11', working_hours: 'Пн–Сб 9:00–19:00',  open_on_weekends: true,  rank: 1, isFeature: true  },
    { name: 'Ростов Газон',          city: cities[8],  phone: '+7 (863) 400-50-60', working_hours: 'Пн–Пт 9:00–18:00',  open_on_weekends: false, rank: 1, isFeature: false },
    { name: 'ЮгГрин Ростов',         city: cities[8],  phone: '+7 (863) 700-80-90', working_hours: 'Пн–Вс 8:00–20:00',  open_on_weekends: true,  rank: 2, isFeature: true  },
    { name: 'УфаГазон',              city: cities[9],  phone: '+7 (347) 111-33-55', working_hours: 'Пн–Пт 9:00–18:00',  open_on_weekends: false, rank: 1, isFeature: false },
    { name: 'Башкирский газон',      city: cities[9],  phone: '+7 (347) 600-70-80', working_hours: 'Пн–Сб 9:00–20:00',  open_on_weekends: true,  rank: 2, isFeature: true  },
    { name: 'ПермьГрин',             city: cities[10], phone: '+7 (342) 500-60-70', working_hours: 'Пн–Пт 9:00–18:00',  open_on_weekends: false, rank: 1, isFeature: false },
    { name: 'Воронеж Газон',         city: cities[11], phone: '+7 (473) 800-90-01', working_hours: 'Пн–Сб 9:00–19:00',  open_on_weekends: true,  rank: 1, isFeature: true  },
  ];
